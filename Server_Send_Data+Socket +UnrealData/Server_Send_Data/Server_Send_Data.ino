...
void loop() {
 ...
 {    
      ...
      //準備傳送資料
      Udp.beginPacket("192.168.137.1", 2333);
      for(int i = 0 ; i < Sensor_Count ; i ++ )
        for(int j = 0 ; j < 6 ; j++ )
        {
          Udp.print(Final_Motion_Data[i][j]);
          Udp.print(",");
        }
      //Udp.write((uint8_t *)&Final_Motion_Data , sizeof(Final_Motion_Data));
      Serial.println(Final_Motion_Data[0][0]);//複製資料到傳送快取
      Udp.endPacket();
